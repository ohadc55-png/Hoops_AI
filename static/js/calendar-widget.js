...
class CalendarWidget {
  ...
  render() {
    ...
    let html = `<div class="calendar">
      <div class="calendar-header">
        <div class="calendar-nav">
          ...
          <button onclick="window._calWidgets['${wid}'].goToToday()" title="Today" style="width:auto;padding:0 10px;font-size:var(--text-xs);">Today</button>
    ...
    if (this.showAddButton && this.onAddEvent) {
      html += `<button class="btn btn-primary btn-sm" onclick="window._calWidgets['${wid}'].onAddEvent()"><span class="material-symbols-outlined" style="font-size:18px;">add</span> Add Event</button>`;
    }
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      dayEvents.slice(0, maxPills).forEach(e => {
        pills += `<span class="calendar-event-pill ${this.pillClass(e)}">${this._esc(e.title)}</span>`;
      });
      if (dayEvents.length > maxPills) {
        pills += `<span class="calendar-event-more">+${dayEvents.length - maxPills} more</span>`;
      }
    ...
    if (this.selectedDate) {
      ...
      if (this.showAddButton && this.onAddEvent) {
        html += `<button class="btn btn-primary btn-sm" onclick="window._calWidgets['${wid}'].onAddEvent('${this.selectedDate}')"><span class="material-symbols-outlined" style="font-size:18px;">add</span> Add</button>`;
      }

      html += '</div>';

      if (selEvents.length === 0) {
        html += '<div class="calendar-detail-empty">No events on this day</div>';
      } else {
        selEvents.forEach(e => {
          const recurring = e.recurrence_group ? '<span class="recurring-badge"><span class="material-symbols-outlined">repeat</span>Series</span>' : '';
          const isAdmin = e.source === 'admin';
          const adminBadge = (isAdmin && this.showSourceBadge) ? '<span class="admin-event-badge">From Admin</span>' : '';

          html += `<div class="calendar-detail-event${isAdmin ? ' admin-event' : ''}">
            <div class="event-dot ${this.pillClass(e)}"></div>
            <div class="event-info">
              <div class="event-title">${this._esc(e.title)} ${recurring} ${adminBadge}</div>
              <div class="event-meta">${this._capitalize(e.event_type)} ${e.time ? '&middot; ' + e.time : ''} ${e.time_end ? '- ' + e.time_end : ''} ${e.opponent ? '&middot; vs ' + this._esc(e.opponent) : ''} ${e.location ? '&middot; ' + this._esc(e.location) : ''}</div>
              ${e.notes ? '<div class="event-meta">' + this._esc(e.notes) + '</div>' : ''}
              ${e.is_away ? `<div class="transport-info"><span class="material-symbols-outlined" style="font-size:14px;vertical-align:middle;color:#F87171;">directions_bus</span> <strong style="color:#F87171;">Away Game</strong>${e.departure_time ? ' &middot; Departure: <strong>' + this._esc(e.departure_time) + '</strong>' : ''}${e.venue_address ? '<div style="margin-top:2px;font-size:var(--text-xs);color:var(--text-secondary);">' + this._esc(e.venue_address) + '</div><div style="display:flex;gap:var(--sp-3);margin-top:4px;"><a href="https://waze.com/ul?q=' + encodeURIComponent(e.venue_address) + '&navigate=yes" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:4px;font-size:var(--text-xs);color:#33ccff;text-decoration:none;font-weight:600;"><img src="https://www.waze.com/favicon.ico" width="14" height="14" style="border-radius:2px;" alt=""> Navigate with Waze</a><a href="https://www.google.com/maps/search/' + encodeURIComponent(e.venue_address) + '" target="_blank" rel="noopener" style="font-size:var(--text-xs);color:var(--primary-light);text-decoration:none;">Google Maps &rarr;</a></div>' : ''}</div>` : ''}
            </div>`;
