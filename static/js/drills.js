...
function renderDrills(drills) {
  ...
  if (!drills.length) {
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><span class="material-symbols-outlined">fitness_center</span><h3>${t('drills.empty.title')}</h3><p>${t('drills.empty.subtitle')}</p></div>`;
    return;
  }
  ...
  grid.innerHTML = drills.map(d => `
    <div class="card card-interactive" onclick="viewDrill(${d.id})">
      ...
      <div class="flex items-center justify-between text-xs text-muted">
        <span>${t('drills.label.min', { count: d.duration_minutes })}</span>
        <div class="flex gap-2 items-center">
          ${d.video_url ? `<span class="badge badge-neutral" style="font-size:9px;">${t('drills.badge.video')}</span>` : ''}
          ${d.is_ai_generated ? `<span class="badge badge-primary" style="font-size:9px;">${t('drills.badge.ai')}</span>` : ''}
          ${d.assignment_count > 0 ? `<span class="badge badge-warning" style="font-size:9px;">${t('drills.badge.assigned', { count: d.assignment_count })}</span>` : ''}
        </div>
      </div>
      <div style="margin-top:var(--sp-3);padding-top:var(--sp-3);border-top:1px solid var(--border-subtle, rgba(255,255,255,0.06));display:flex;justify-content:flex-end;">
        <button class="btn btn-secondary" style="font-size:var(--text-xs);padding:var(--sp-1) var(--sp-3);border-color:var(--primary);color:var(--primary);" onclick="event.stopPropagation();openAssignModal(${d.id})">
          <span class="material-symbols-outlined" style="font-size:16px;">assignment_ind</span> ${t('drills.btn.assign')}
        </button>
      </div>
    </div>
  `).join('');
...
async function createDrill() {
  try {
    ...
    Toast.success(t('drills.created'));
    loadDrills();
  } catch (e) { /* handled */ }
}
...
async function generateDrill() {
  ...
  const textEl = btn.querySelector('[data-i18n]') || btn.lastElementChild;
  if (iconEl) iconEl.textContent = 'hourglass_empty';
  if (textEl) textEl.textContent = t('drills.generate.loading');
  try {
    ...
    Toast.success(t('drills.generate.success'));
    loadDrills();
  } catch (e) { /* handled */ }
  finally {
    btn.disabled = false;
    if (iconEl) iconEl.textContent = 'auto_awesome';
    if (textEl) textEl.textContent = t('drills.generate.btn');
  }
}
...
async function viewDrill(id) {
  try {
    ...
    const videoHtml = embedUrl
      ...
      : (d.video_url ? `<div style="margin-bottom:var(--sp-4);"><a href="${escHtml(d.video_url)}" target="_blank" rel="noopener" style="color:var(--primary);font-size:var(--text-sm);display:flex;align-items:center;gap:4px;"><span class="material-symbols-outlined" style="font-size:18px;">play_circle</span> ${t('drills.detail.watch_video')}</a></div>` : '');

    document.getElementById('detailBody').innerHTML = `
      <div class="flex gap-2" style="margin-bottom:var(--sp-4);">
        <span class="badge badge-neutral">${capitalize(d.category)}</span>
        <span class="badge badge-${diffColors[d.difficulty] || 'neutral'}">${capitalize(d.difficulty)}</span>
        <span class="badge badge-neutral">${t('drills.label.min', { count: d.duration_minutes })}</span>
        ${d.is_ai_generated ? `<span class="badge badge-primary">${t('drills.detail.ai_generated')}</span>` : ''}
      </div>
      ${videoHtml}
      ${d.description ? `<p style="margin-bottom:var(--sp-4);color:var(--text-secondary);">${escHtml(d.description)}</p>` : ''}
      ${d.instructions ? `<div style="margin-bottom:var(--sp-4);"><h4 style="font-weight:600;margin-bottom:var(--sp-2);">${t('drills.detail.instructions')}</h4><pre style="white-space:pre-wrap;font-size:var(--text-sm);color:var(--text-secondary);background:var(--bg-card);padding:var(--sp-3);border-radius:var(--r-md);">${escHtml(d.instructions)}</pre></div>` : ''}
      ${d.coaching_points?.length ? `<div style="margin-bottom:var(--sp-4);"><h4 style="font-weight:600;margin-bottom:var(--sp-2);">${t('drills.detail.coaching_points')}</h4><ul style="padding-left:var(--sp-5);color:var(--text-secondary);font-size:var(--text-sm);">${d.coaching_points.map(p => `<li style="margin-bottom:var(--sp-1);">${escHtml(p)}</li>`).join('')}</ul></div>` : ''}
      ${d.tags?.length ? `<div class="flex gap-2" style="margin-bottom:var(--sp-4);">${d.tags.map(tag => `<span class="badge badge-neutral">${escHtml(tag)}</span>`).join('')}</div>` : ''}
      <div id="assignmentStatus_${d.id}"></div>
    `;

    document.getElementById('detailFooter').innerHTML = `
      <button class="btn btn-secondary" onclick="deleteDrill(${d.id})"><span class="material-symbols-outlined" style="font-size:18px;">delete</span></button>
      <button class="btn btn-secondary" onclick="openAssignModal(${d.id})" style="border-color:var(--primary);color:var(--primary);"><span class="material-symbols-outlined" style="font-size:18px;">assignment_ind</span> ${t('drills.btn.assign')}</button>
      <button class="btn btn-primary" onclick="closeModal('drillDetailModal')">${t('btn.close')}</button>
    `;
...
async function loadAssignmentStatus(drillId) {
  ...
  try {
    ...
    if (!assignments.length) {
      container.innerHTML = `<p style="font-size:var(--text-sm);color:var(--text-muted);margin-top:var(--sp-2);">${t('drills.assign.not_assigned')}</p>`;
      return;
    }
    container.innerHTML = `
      <h4 style="font-weight:600;margin-bottom:var(--sp-2);display:flex;align-items:center;gap:var(--sp-2);">
        ...
        ${t('drills.assignments.title')} (${assignments.length})
      </h4>
      <div style="max-height:200px;overflow-y:auto;">
        ${assignments.map(a => {
          const statusMap = {
            'approved': { color: 'var(--success, #22c55e)', text: t('drills.status.approved'), icon: 'check_circle' },
            'video_uploaded': { color: 'var(--info, #3b82f6)', text: t('drills.status.video_pending'), icon: 'rate_review' },
            'rejected': { color: 'var(--error, #ef4444)', text: t('drills.status.rejected'), icon: 'close' },
            'pending': { color: 'var(--warning, #f59e0b)', text: t('drills.status.pending'), icon: 'schedule' },
          };
...
async function openAssignModal(drillId) {
  ...
  listEl.innerHTML = `<div style="text-align:center;padding:var(--sp-4);color:var(--text-muted);">${t('drills.assign.loading_players')}</div>`;
  ...
  try {
    ...
    if (!players.length) {
      listEl.innerHTML = `<div style="text-align:center;padding:var(--sp-4);color:var(--text-muted);">${t('drills.assign.no_players')}</div>`;
      return;
    }
    ...
    listEl.innerHTML = `<div style="text-align:center;padding:var(--sp-4);color:var(--text-muted);">${t('drills.assign.load_failed')}</div>`;
  }
}
...
async function submitAssignment() {
  ...
  if (!checked.length) {
    Toast.error(t('drills.assign.select_player'));
    return;
  }
  try {
    ...
    Toast.success(t('drills.assign.success', { count: res.data.assigned }));
    loadAssignmentStatus(_assignDrillId);
    loadDrills();
  } catch (e) {
    Toast.error(t('drills.assign.failed'));
  }
}
...
async function deleteDrill(id) {
  if (!confirm(t('drills.confirm_delete'))) return;
  try {
    await API.del(`/api/drills/${id}`);
    closeModal('drillDetailModal');
    Toast.success(t('drills.deleted'));
    loadDrills();
  } catch(e) {}
}
...
async function openTrackingModal() {
  ...
  body.innerHTML = `<div style="text-align:center;padding:var(--sp-6);color:var(--text-muted);">${t('drills.tracking.loading')}</div>`;
  ...
  try {
    ...
    const players = res.data || [];
    if (!players.length) {
      body.innerHTML = `<div style="text-align:center;padding:var(--sp-6);color:var(--text-muted);">${t('drills.tracking.empty').replace('\n', '<br>')}</div>`;
      return;
    }
    ...
    for (const [teamName, teamPlayers] of Object.entries(teams)) {
      ...
      html += `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--sp-3);padding-bottom:var(--sp-2);border-bottom:1px solid var(--border-subtle, rgba(255,255,255,0.06));">
        ...
        <span style="font-size:var(--text-sm);color:var(--text-secondary);">${teamCompleted}/${teamAssigned} ${t('drills.tracking.completed')} (${teamPct}%)</span>
      </div>`;
      ...
      for (const p of teamPlayers) {
        ...
        const lastActivity = p.last_completed ? timeAgoTracking(p.last_completed) : t('drills.tracking.never');
    ...
    body.innerHTML = `<div style="text-align:center;padding:var(--sp-6);color:var(--text-muted);">${t('drills.tracking.failed')}</div>`;
  }
}

function timeAgoTracking(dateStr) {
  if (!dateStr) return t('drills.tracking.never');
  const d = new Date(dateStr.endsWith('Z') ? dateStr : dateStr + 'Z');
  const diff = (Date.now() - d.getTime()) / 1000;
  if (diff < 0) return t('drills.tracking.just_now');
...
async function openReviewsModal() {
  ...
  try {
    ...
    if (!reviews.length) {
      body.innerHTML = `<div style="text-align:center;padding:var(--sp-6);color:var(--text-muted);"><span class="material-symbols-outlined" style="font-size:40px;display:block;margin-bottom:var(--sp-2);">check_circle</span>${t('drills.reviews.none')}</div>`;
      return;
    }
    ...
    body.innerHTML = reviews.map(r => `
      <div class="review-item" id="review-${r.assignment_id}" style="padding:var(--sp-4);margin-bottom:var(--sp-3);background:var(--bg-card, rgba(255,255,255,0.03));border:1px solid rgba(255,255,255,0.06);border-radius:var(--r-lg);">
        ...
        <div class="input-group" style="margin-bottom:var(--sp-3);">
          <label style="font-size:var(--text-sm);color:var(--text-secondary);">${t('drills.reviews.feedback_label')}</label>
          <textarea class="textarea" id="feedback-${r.assignment_id}" placeholder="${t('drills.reviews.feedback_placeholder')}" rows="2" style="background:var(--bg-input, rgba(255,255,255,0.05));border:1px solid rgba(255,255,255,0.08);border-radius:var(--r-md);padding:var(--sp-2) var(--sp-3);color:var(--text-primary);font-size:var(--text-sm);width:100%;resize:vertical;"></textarea>
        </div>
        <div style="display:flex;gap:var(--sp-3);justify-content:flex-end;">
          <button class="btn btn-secondary" style="border-color:var(--error, #ef4444);color:var(--error, #ef4444);" onclick="reviewDrill(${r.assignment_id}, 'reject')">
            <span class="material-symbols-outlined" style="font-size:18px;">close</span> ${t('drills.reviews.reject_btn')}
          </button>
          <button class="btn btn-primary" onclick="reviewDrill(${r.assignment_id}, 'approve')">
            <span class="material-symbols-outlined" style="font-size:18px;">check</span> ${t('drills.reviews.approve_btn')}
          </button>
        </div>
      </div>
    `).join('');
  } catch {
    body.innerHTML = `<div style="text-align:center;padding:var(--sp-6);color:var(--text-muted);">${t('drills.reviews.failed')}</div>`;
  }
}
...
async function reviewDrill(assignmentId, action) {
  ...
  try {
    ...
    Toast.success(action === 'approve' ? t('drills.reviews.approved') : t('drills.reviews.rejected'));
    ...
    if (item) {
      ...
      setTimeout(() => {
        ...
        if (!remaining.length) {
          document.getElementById('reviewsBody').innerHTML = `<div style="text-align:center;padding:var(--sp-6);color:var(--text-muted);"><span class="material-symbols-outlined" style="font-size:40px;display:block;margin-bottom:var(--sp-2);">check_circle</span>${t('drills.reviews.all_complete')}</div>`;
        }
        updateReviewBadge();
      }, 250);
    }
    ...
    Toast.error(t('drills.reviews.failed_action'));
  }
}
