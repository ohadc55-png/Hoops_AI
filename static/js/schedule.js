...
async function loadMyTeams() {
  ...
  if (sel) {
    sel.innerHTML = _myTeams.map(t => `<option value="${t.id}">${esc(t.name)}</option>`).join('');
...
function openEventModal(data, prefillDate) {
  ...
  sel.innerHTML = '<option value="">None</option>' + _facilities.map(f =>
...
async function saveEvent() {
  ...
  if (!body.title || !body.date) { Toast.error('Title and date are required'); return; }
  try {
    if (id) {
      ...
      Toast.success(repeat ? `Created ${repeat} recurring events!` : 'Event created!');
    }
...
async function deleteEvent(id) {
  ...
  try { await API.del(`/api/events/${id}`); Toast.success('Event deleted'); _calWidget.refresh(); } catch (e) {}
}

async function deleteEventSeries(group) {
  if (!confirm('Delete ALL events in this recurring series?')) return;
  try { await API.del(`/api/events/series/${group}`); Toast.success('Series deleted'); _calWidget.refresh(); } catch (e) {}
}
...
async function submitRequest() {
  ...
  if (!body.title || !body.date) { Toast.error('Title and date are required'); return; }
  if (!body.team_id) { Toast.error('No team selected'); return; }
  ...
  try {
    ...
    let html = `<div class="card" style="padding:var(--sp-4);">
      <h3 style="margin-bottom:var(--sp-3);display:flex;align-items:center;gap:var(--sp-2);">
        <span class="material-symbols-outlined">pending_actions</span> My Schedule Requests
      </h3>`;
