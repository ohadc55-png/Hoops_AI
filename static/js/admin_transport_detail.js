...
async function loadEventDetail() {
  ...
  try {
    ...
    cardEl.innerHTML = '<div class="empty-state-admin">Could not load event details</div>';
  }
}
...
function renderEventInfo() {
  ...
  document.getElementById('eventInfoCard').innerHTML = `
    <div class="section-header">
      ...
      <span class="badge" style="background:rgba(248,113,113,0.15);color:#F87171;padding:4px 10px;border-radius:6px;">Away Game</span>
    </div>
    <div class="section-content">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--sp-4);">
        <div>
          <div style="color:var(--text-muted);font-size:var(--text-xs);text-transform:uppercase;margin-bottom:2px;">Date</div>
          <div style="font-weight:600;">${dateStr}</div>
        </div>
        <div>
          <div style="color:var(--text-muted);font-size:var(--text-xs);text-transform:uppercase;margin-bottom:2px;">Team</div>
          <div style="font-weight:600;">${esc(e.team_name)}</div>
        </div>
        <div>
          <div style="color:var(--text-muted);font-size:var(--text-xs);text-transform:uppercase;margin-bottom:2px;">Game Time</div>
          <div style="font-weight:600;">${e.time_start || '-'}${e.time_end ? ' - ' + e.time_end : ''}</div>
        </div>
        <div>
          <div style="color:var(--text-muted);font-size:var(--text-xs);text-transform:uppercase;margin-bottom:2px;">Opponent</div>
          <div style="font-weight:600;">${e.opponent ? esc(e.opponent) : '-'}</div>
        </div>
        <div>
          <div style="color:var(--text-muted);font-size:var(--text-xs);text-transform:uppercase;margin-bottom:2px;">Departure Time</div>
          <div style="font-weight:600;color:${e.departure_time ? '#34D399' : 'var(--text-muted)'};">
            ...
            ${e.departure_time || 'Not set'}
          </div>
        </div>
        <div>
          <div style="color:var(--text-muted);font-size:var(--text-xs);text-transform:uppercase;margin-bottom:2px;">Venue Address</div>
          <div style="font-weight:600;">
            <span class="material-symbols-outlined" style="font-size:16px;vertical-align:middle;">location_on</span>
            ${e.venue_address ? esc(e.venue_address) : 'Not set'}
          </div>
        </div>
      </div>
      ${e.location ? `<div style="margin-top:var(--sp-3);color:var(--text-secondary);font-size:var(--text-sm);">Location: ${esc(e.location)}</div>` : ''}
      ${e.notes ? `<div style="margin-top:var(--sp-2);color:var(--text-secondary);font-size:var(--text-sm);">Notes: ${esc(e.notes)}</div>` : ''}
    </div>
  `;
}
...
function renderMap(address) {
  ...
  navButtons.innerHTML = `
    <a href="https://waze.com/ul?q=${encoded}&navigate=yes" target="_blank" rel="noopener"
      ...
      <span class="material-symbols-outlined" style="font-size:16px;">navigation</span> Navigate with Waze
    </a>
    <a href="https://www.google.com/maps/search/${encoded}" target="_blank" rel="noopener"
      ...
      <span class="material-symbols-outlined" style="font-size:16px;">map</span> Google Maps
    </a>`;
...
function updateMap() {
  ...
  if (address) {
    ...
    AdminToast.error('Enter a venue address first');
  }
}
...
async function saveTransportInfo(e) {
  ...
  try {
    ...
    AdminToast.success('Transport info updated & team notified');
    // Reload details
    loadEventDetail();
  } catch {
